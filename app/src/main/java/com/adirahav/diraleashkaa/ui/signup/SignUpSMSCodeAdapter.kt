...
class SignUpSMSCodeAdapter(
    private val listener: SignUpPersonalInfoFragment
) : RecyclerView.Adapter<SignUpSMSCodeAdapter.ViewHolder>() {
            ...
            override fun afterTextChanged(s: Editable?) {
                if (s.toString().length == 1) {
                    if (holder.absoluteAdapterPosition < itemCount.minus(1)) {
                        ...
                        val send = listener.smsDialog?.findViewById<Button>(R.id.send)
                        Utilities.setButtonEnable(send)
                        //listener.checkSMSCode()
                    }
                }
            }
