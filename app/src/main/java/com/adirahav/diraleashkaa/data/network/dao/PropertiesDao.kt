...
interface PropertiesDao {
    ...
    @Query("SELECT * FROM propertyEntity WHERE archive is not 1")
    fun getAll(): List<PropertyEntity>//LiveData<List<PropertyEntity>>

    @Query("SELECT * FROM propertyEntity WHERE archive is 1")
    fun getAllArchive(): List<PropertyEntity>

    //@Query("SELECT city, COUNT(*) FROM propertyEntity GROUP BY city ORDER BY COUNT(*) DESC")
    @Query("SELECT city, COUNT(*) FROM propertyEntity WHERE archive is not 1 and roomID is not 0 GROUP BY city")
    fun getMyCities(): List<PropertyEntity>//LiveData<List<PropertyEntity>>

    @Query("SELECT * FROM propertyEntity WHERE city is :city and roomID is not 0 and archive is not 1")
