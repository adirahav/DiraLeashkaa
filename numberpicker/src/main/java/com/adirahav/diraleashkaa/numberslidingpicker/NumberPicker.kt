package com.adirahav.diraleashkaa.numberpickerlibrary
...
class NumberPicker @JvmOverloads constructor(
    ...
    fun setProgress(value: Float, fromUser: Boolean = true) {
        ...
        if (value > -1 && value != data.value) {
            data.value = value

            val formatValue =
                if (editText.text.toString().isValidInt())
                    value.toString()
                else
                    String.format(if (data.stepSize == 1.0f) "%.0f" else "%.1f", data.value)

            tooltip?.update(formatValue)
    ...
    init {
        ...
        try {
            ...
            val stepSize = 0.1f//array.getFloat(R.styleable.NumberPicker_picker_stepSize, 1f)
            val orientation = array.getInteger(R.styleable.NumberPicker_picker_orientation, LinearLayout.VERTICAL)
            val value = array.getFloat(R.styleable.NumberPicker_android_progress, 0f)
            ...
            val tracker_type = array.getFloat(R.styleable.NumberPicker_picker_tracker, TRACKER_LINEAR)
    ...
    private fun inflateChildren() {
        ...
        editText.setTextColor(Color.parseColor("#828487"))
    ...
    private fun initializeButtonActions() {
        ...
        editText.doOnTextChanged { text, _, _, _ ->
            if (!text.isNullOrEmpty()) {
                try {
                    setProgress(editText.text.toString().toFloat(), true)
        ...
        editText.setOnFocusChangeListener { _, hasFocus ->
            ...
            if (!hasFocus) {
                if (!editText.text.isNullOrEmpty() && (editText.text.indexOf('.', editText.text.indexOf('.') + 1) != -1)) {
                    setProgress(editText.text.toString().toFloat(), true)
                }
                else {
                    editText.setText(data.value.toString())
    ...
    companion object {
        ...
        const val TRACKER_LINEAR = 0f
        const val TRACKER_EXPONENTIAL = 1f
